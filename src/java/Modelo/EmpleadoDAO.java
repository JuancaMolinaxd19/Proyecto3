...
public class EmpleadoDAO {
    ...
    public int agregar(Empleado em){ 
        String sql="insert into empleado(dpi, nombres, telefono,estado,usuario)values(?,?,?,?,?)";
        try {
            con=cn.Conexion();
            ps=con.prepareStatement(sql);
    ...
    public int actualizar(Empleado em){
        String sql="update empleado set dpi=?, nombres=?, telefono=?,estado=?,usuario=? where IdEmpleado=?";
        try {
            con=cn.Conexion();
            ps=con.prepareStatement(sql);
            ps.setString(1, em.getDPI());
